...
- 启禁用状态：`users` 表已有 `status` 字段（schema 见 `10-服务端/03-数据库Schema说明.md`，迁移 `20260912000322_phase2_add_user_status/`），运营通过后台把 `status` 改成 `0` 即可标记禁用。
- 后续生产化阶段：可在 `AuthService.login` 增加 `user.status !== 1` 校验，禁止被禁用的用户登录；当前版本仅在后台列表层过滤，不阻断前台登录。
