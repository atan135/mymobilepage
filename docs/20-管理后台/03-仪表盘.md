...
```ts
{
  ...
  topProducts: Array<{             // 热销 Top10，按 sales desc
    productId: number               // 注：原文档写 id，实际为 productId
    title: string
    cover: string
    sales: number
    gmv: number                     // 注：原文档无 gmv 列
    id: number
    title: string
    cover: string
    sales: number
...
> 交互：第 1 / 第 4 个卡（待处理订单 / 库存预警）支持 `@click` 跳转；其他卡纯展示。
> 顶部右侧「查看全部 →」链接指向对应列表页。
## 6. 销售趋势折线图（ECharts）
...
- **默认展示 7 天，写死 `getSalesTrend(7)`**（当前实现无切换控件，文档原"可改 14/30"为占位描述）
- X 轴是日期字符串，Y 轴是金额
- 区域渐变 + 平滑曲线
...
> 数据按 `sales desc` 取前 10；后端在 `AdminDashboardService.overview` 中实现。返回结构：`{ productId, title, cover, sales, gmv }`，无价格/库存列。
...
> 点击行直接跳 `/orders`（管理员处理）。当前表格不含「创建时间」列（与 doc 早期版本不一致，已移除）。
