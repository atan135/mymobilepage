...
| 方法 | 路径 | 权限码 | 说明 |
| --- | --- | --- | --- |
| `GET` | `/api/admin/settings` | `setting:edit` | 列出所有设置（按 group / key 排序） |
| `PUT` | `/api/admin/settings` | `setting:edit` | 批量 upsert：body `{ updates: [{ key, value }] }` |
...
| 权限码 | 用途 | ADMIN 默认 |
| --- | --- | --- |
| `setting:edit` | 后台查看 / 编辑全部设置 | ✓ |

客户端公共读无需权限（`@Public`）。

`types.ts` 中占位的 `setting:edit` 直接用上。
