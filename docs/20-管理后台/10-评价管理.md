...
> Phase 2 / 2.3 模块 + Phase 3 客户端入口。`Review` 表 + 后台审核 / 回复能力 + 前台提交评价入口（Phase 3 补齐）。
...
- **用户（client）**：在订单 `status=3`（已完成）后对订单内每个商品写1 次评价（1~5 星 + 内容 + 图片），入口见 `docs/30-移动端/06-商品评价.md`
...
### 前台接口（`server/src/client-reviews/`）

| 方法 | 路径 | 鉴权 | 说明 |
| --- | --- | --- | --- |
| `POST` | `/api/reviews` | `JwtAuthGuard` | 创建评价（订单已付款 + 商品归属 + 每订单每商品唯一） |
| `GET` | `/api/reviews/my` | `JwtAuthGuard` | 我的评价（按 `status` / 分页） |
| `GET` | `/api/reviews/can-review/:orderId` | `JwtAuthGuard` | 订单内每件商品是否可评价 |

详情见 `docs/30-移动端/06-商品评价.md` 第 3 节。
...
- 客户端评价入口见 `docs/30-移动端/06-商品评价.md`；seed 仍写入 11 条 demo 评价（`orderId=null` 不参与唯一约束，可并存）
- 一订单对一商品仅允许 1 条评价：`@@unique([orderId, productId])` 索引 `uniq_review_per_order_product`（Phase 3 新增）
- 不接图片上传到对象存储：`images` 直接以 URL 数组存储（demo 用 `picsum.photos`）
- 不做客户端聚合统计（如「商品平均分」），按需在商品详情 / 列表用 Prisma `groupBy` 实时聚合

## 8. 关联

- 前台模块：`server/src/client-reviews/`（Phase 3 补齐），依赖 `AuthModule` + `PassportModule(jwt)`
- 复用同一张 `reviews` 表，写入默认 `status = 0`（待审）
- 复用 admin-reviews 的 `REVIEW_TRANSITIONS` / `REVIEW_STATUSES` 状态机常量

## 9. 相关文档

- 服务端模块清单：`10-服务端/02-模块总览.md`
- 客户端评价入口：`30-移动端/06-商品评价.md`
