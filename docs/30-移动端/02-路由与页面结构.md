...
| `/order/success` | `OrderSuccessView.vue` | — | 是 | 下单成功（带 `?orderNo`） |
...
- 用户卡片：头像 + 昵称 + 账号 + 手机号
- 菜单列表：「我的订单」「优惠券（Phase 2）」「收货地址（Phase 2）」「客户服务」
...
- 提交：`createOrder({ items, receiver, remark })` → 跳 `/order/success?orderNo=...`
- 购物车来时：清掉已结算的 `cart.clearSelected()`

### OrderSuccessView.vue

- 显示订单号（从 `route.query.orderNo`）
