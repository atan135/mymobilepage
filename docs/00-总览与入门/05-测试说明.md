...
12. 底部出现「评价『商品名』」按钮（每个未评商品各一个）→ 点进入 `/review/create`
13. 选 5 星 + 填写评价内容（可选粘图片 URL）→ 「提交评价」 → `showToast`「评价已提交，待审核」→ 跳回订单详情；「评价」按钮消失
14. 切到后台 `operator / admin123` → 「评价管理」列表 → 该评价出现在「待审」Tab → 详情点「通过」→ status 变 1
...
**Seed 写 3 张券**（id=101~103）：101 新人立减券（无门槛减 10）+ 102 满 99 减 20（满减）+ 103 满 200 打 85 折（折扣），状态全 1、有效期未来 30 天；同时给 alice/bob/carol 各发 1 张 101（管理员发放），并预置 alice 一张已使用的 102、dave 一张已过期的 102，可演示「我的」三个状态 Tab。
...
| 用例 | UI 位置 | 期望 |
| --- | --- | --- |
| 评分 Tab 切换 | 列表页 `/reviews` 第二个 tab 区（card 风格）：全部 / 1 星 / 2 星 / 3 星 / 4 星 / 5 星 | 列表按评分过滤 |
| 关键字搜索 | 列表页 `/reviews` 顶部 `el-form` 搜索框（占位符「评价内容 / 商品标题 / 用户名」） | `content` / 商品标题 / 用户名 三字段命中 |
| 「审核通过」 | 详情页 `/reviews/:id` 底部按钮区。**仅 `status === 0` 显示「通过」**；`status === 2` 时同一位置显示「恢复为通过」，底层都走 `PATCH /:id/approve` | status 0 → 1；状态 2 走恢复路径 → 1 |
| 「屏蔽」 | 详情页 `/reviews/:id` 底部按钮区。`status ∈ {0, 1}` 时显示「屏蔽」（`PATCH /:id/block`） | status 0 → 2；状态 1 → 2 |
| 「回复」 | 详情页 `/reviews/:id` 底部按钮区「写回复 / 编辑回复」→ 弹 dialog（textarea，留空保存 = 清除） | 空串视为清除（trim 后 `reply: null`） |
| 跨级跳（1 → 2 → 0） | 服务端 `AdminReviewsService.assertTransition` 兜底（前端不暴露非法按钮） | 400「评价状态不允许从 X 切换到 Y」 |
| 客户端提交评价 | `/order/detail?id=...` status=3 时点「评价『商品名』」→ `/review/create` 填表提交 → 后台新行出现在「待审」 | status=0；DB 唯一约束防重复 |

> 列表页行内只有「详情」按钮，所有审核 / 屏蔽 / 回复动作都在详情页完成。按钮按 `canApprove` / `canBlock` / `canRestore` 计算显隐，因此测试时需按状态进入对应单据。
