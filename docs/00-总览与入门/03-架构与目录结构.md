...
### 服务端 25 个业务模块（详见 `10-服务端/02-模块总览.md`）

- **基础**：`PrismaModule`（全局）、`ConfigModule`（全局）、`AuditModule`（全局 `AuditInterceptor`）
- **公共**：`AuthModule`（前台 JWT）、`UsersModule`（前台）、`InventoryModule`（库存变更流水公共封装）
- **客户端公开**：`CategoriesModule`、`ProductsModule`、`ClientBannersModule`、`ClientSettingsModule`
- **客户端登录后**：`ClientOrdersModule`（JwtAuthGuard）、`CouponsModule`（JwtAuthGuard）、`RefundsModule`（JwtAuthGuard）
- **后台**：`AdminAuthModule`、`AdminUsersModule`、`AdminCategoriesModule`、`AdminProductsModule`、`AdminBannersModule`、`AdminAnnouncementsModule`、`AdminOrdersModule`、`AdminDashboardModule`、`AdminCouponsModule`、`AdminRefundsModule`、`AdminReviewsModule`、`AdminInventoryModule`、`AdminSettingsModule`、`AdminAuditLogsModule`、`AdminExportsModule`

### 管理后台 8 个页面（详见 `20-管理后台/`）

- 登录 / 仪表盘 / 用户 / 分类 / 商品 / 订单 / 轮播图 / 公告

### 移动端 13 个页面（详见 `30-移动端/02-路由与页面结构.md`）

- 登录 / 首页（Tab）/ 分类（Tab）/ 购物车（Tab）/ 我的（Tab）/ 商品详情 / 订单确认 / 下单成功 / 我的订单 / 订单详情 / 领券中心 / 我的退款 / 退款详情
...
| `categories` | 商品分类 | 初始 |
| `products` | 商品（含 `threshold`） | 初始 + 2 |
| `banners` | 首页轮播图 | 初始 |
| `announcements` | 首页公告 | 1 |
| `orders` | 订单（含 `original_amount` / `discount_amount` / `refunded_at`） | 1 + 2 |
| `order_items` | 订单商品快照 | 1 |
| `refund_requests` | 退款申请（含状态机） | 1 + 2 |
| `coupons` | 优惠券 | 2 |
| `user_coupons` | 用户领取的券 | 2 |
| `reviews` | 商品评价 | 2 |
| `inventory_logs` | 库存变更流水 | 2 |
| `audit_logs` | 后台操作审计 | 2 |
| `settings` | 系统设置 KV | 2 |
