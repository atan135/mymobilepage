...
来源：`server/prisma/seed.ts` 的 `ADMIN_PERMISSIONS` 数组（41 条）。
...
> 以下权限码在 `types.ts` 中声明、controller 上有 `@RequirePermission()`，但 seed 中**未分配**给 ADMIN（仅 SUPER_ADMIN 通过 `*` 通配可访问）：
>
> - `user:edit` / `category:delete` / `product:delete`
> - `order:cancel` / `banner:delete` / `announcement:publish` / `announcement:delete`
>
> 这是 known issue：UI 上对应按钮的 `v-permission` 会隐藏，但前端若绕过指令直接调接口会被 `PermissionsGuard` 拦截。生产化阶段需统一决策：要么补 seed 授权、要么移除 controller 装饰器。

- `SUPER_ADMIN` 角色：`permissions = ['*']` → 守卫中 `permissions.includes('*')` 直接放行
- `ADMIN` 角色：上述 41 条（含部分敏感操作如 `coupon:delete` / `refund:approve` / `refund:refund`）
...
| `CouponsController` | **整个控制器** `@UseGuards(JwtAuthGuard)` |
| `RefundsController` | **整个控制器** `@UseGuards(JwtAuthGuard)` |
| `ClientSettingsController` | `@Public()`（公开读） |
